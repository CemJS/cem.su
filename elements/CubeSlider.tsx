...
export default function ({ items, key = "" }) {
  class Gallery {
    ...
    constructor(
      ...
      this.currentSlide = currentSlide ? currentSlide : 0;
    ...
    setParameters() {
      if (this.cube && this.cube?.style) {
        ...
        try {
          this.size > 1 && this.cube?.style?.cursor
          ...
          if (this.size > 1 && this.counter) {
            this.counter.innerHTML = `${this.currentSlide + 1} / ${this.size}`;
          ...
          // счётчик
          this.counter && this.size > 1 && this.lineNode
    ...
    destroyEvents() {
      if (this.cube && this.cube?.style) {
        ...
        this.lineNode?.removeEventListener("pointerdown", (e) =>
          e.stopImmediatePropagation(),
        );
    ...
    dragging(e) {
      ...
      if (this.size > 1) {
        if (this.currentSlide + 1 == this.size) {
          this.dragShift = Math.max(
            Math.min(difference, this.widthContainer),
            0,
          );
  ...
  const init = function (element: HTMLElement) {
    Static?.[`galleryInstance${key}`]?.destroyEvents();

    let currentSlide = Static?.[`galleryInstance${key}`]?.currentSlide ?? 0;

    Static[`galleryInstance${key}`] = new Gallery(
      element,
      element.querySelector("#cube"),
