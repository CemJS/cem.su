			...
			<div class="questions-show">
				<div class="wrapper">
					<div class="questions-show__container">
						<div class="questions__user">
							<a class="avatar" href="#">
								<div class="avatar__icon">
									<img class="avatar__photo" src={Static.record.author.avatar?.name ? `/assets/upload/avatar/${Static.record.author.avatar?.name}` : image} />
									<img class="avatar__frame" src={Static.record.author.frame?.name ? `/contents/images/lenta/${Static.record.author.frame?.name}` : frameDefault} />
									{Static.record.author.status?.team ? (
										<img class="avatar__team" src={Static.record.author.status?.team ? teamLogo : null} />
									) : (
										<div>
						...
						<p ref="itemText" class="questions-show__description" init={() => (Ref.itemText.innerHTML = Static.record.text)}></p>
						...
						{Static.record.answers?.length > 0 ? (
							<div class="user-comment__list questions-show__list" ref="answerList">
								{Static.record.answers?.map((answer) => {
									...
									return (
										<div class={["questions-show__item", answer.best ? "questions-show__item_best" : null]}>
											<div class="user-comment__item">
												<a class="avatar" href="">
													<div class="avatar__icon">
														<img class="avatar__photo" src={answer.author.avatar?.name ? `/assets/upload/avatar/${answer.author.avatar?.name}` : image} />
														<img class="avatar__frame" src={answer.author.frame?.name && answer.author.frame?.name != "default.svg" ? `/contents/images/lenta/${answer.author.frame?.name}` : frameDefault} />
														{answer.author.status?.team ? (
															<img class="avatar__team" src={answer.author.status?.team ? teamLogo : null} />
														) : (
															<div>
																<div class="avatar__level">
																	<img src={leveGray} />
												...
												<div class="user-comment__body-big">
													...
													{answer.media.map((item) => {
														return item.type == "image" ? (
															<img src={`/assets/upload/answers/${item.mediaName}`} alt={item.type} class="user-comment__media" />
														) : item.type == "video" ? (
													...
													<div
														...
														onclick={() => {
															...
															if (front.Variable.myInfo.id == Static.record.author.id) {
																!Static.record.closed ? records.push({ name: "Выбрать лучшим", func: () => Func.bestAnswer(answer.id) }) : null;
															}
													...
													return (
														<div class="user-comment__item" style="margin: 0 10px">
															<a class="user-comment__avatar avatar" href="">
																<div class="avatar__icon">
																	<img class="avatar__photo" src={comment.author.avatar?.name ? `/assets/upload/avatar/${comment.author.avatar?.name}` : image} />
																	<img class="avatar__frame" src={comment.author.frame?.name && comment.author.frame?.name != "default.svg" ? `/contents/images/lenta/${comment.author.frame?.name}` : frameDefault} />
																	{comment.author.status?.team ? (
																		<img class="avatar__team" src={comment.author.status?.team ? teamLogo : null} />
																	) : (
																		<div>
																			<div class="avatar__level">
																				<img src={leveGray} />
															...
															{comment.comments?.map((comm, index) => {
																return (
																	<div class="user-comment__item" style="margin: 0 10px">
																		<a class="user-comment__avatar avatar" href="">
																			<div class="avatar__icon">
																				<img class="avatar__photo" src={comm.author.avatar?.name ? `/assets/upload/avatar/${comm.author.avatar?.name}` : image} />
																				<img class="avatar__frame" src={comm.author.frame?.name ? `/contents/images/lenta/${comm.author.frame?.name}` : frameDefault} />
																				{comm.author.status?.team ? (
																					<img class="avatar__team" src={comm.author.status?.team ? teamLogo : null} />
																				) : (
																					<div>
																						<div class="avatar__level">
																							<img src={leveGray} />
