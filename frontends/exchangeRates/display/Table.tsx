    ...
    <table class="relative w-full border-collapse border-inherit text-inherit">
      ...
      <tbody class="block rounded-tl-[--borderR] rounded-tr-[--borderR] text-[1rem] font-medium [border:none] @450:[border:1px_solid_var(--border)]">
        {Static.records?.map((item: exchangersData, index: number) => {
          return (
            <tr class="mb-[.625rem] grid min-h-[5rem] grid-cols-1 items-center max-[450px]:rounded-[--borderR] max-[450px]:bg-[#3b3d49] @374:grid-cols-3 @650:[grid-template-columns:1fr_1fr_2fr_2fr] @450:[&:not(:last-child)]:[border-bottom:1px_solid_var(--border)]">
